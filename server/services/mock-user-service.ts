/**
 * Mock user service implementation using SQLite
...
export class MockUserService implements UserService {
  async register(input: CreateUserInput): Promise<UserResult<User>> {
    ...
    const stmt = db.prepare(`
      INSERT INTO users (id, email, password_hash, name, groups, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
  ...
  async findByEmail(email: string): Promise<User | null> {
    const db = getDatabase();
    const row = db
      .prepare('SELECT id, email, name, groups, created_at, updated_at FROM users WHERE email = ?')
  ...
  async findById(userId: string): Promise<User | null> {
    const db = getDatabase();
    const row = db
      .prepare('SELECT id, email, name, groups, created_at, updated_at FROM users WHERE id = ?')
  ...
  async verifyCredentials(email: string, password: string): Promise<User | null> {
    ...
    const row = db
      ...
      .get(email.toLowerCase()) as DbUserWithPassword | undefined;
